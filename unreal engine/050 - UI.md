...
- To display the UI, add it within any appropriate blueprint (level, player controller, character, camera, etc). This needs the following nodes in order:
  1. **Create HUD Widget**: Creates an instance of the UI element, which should usually be referenced as a variable also.
     1. In a multiplayer game, always check the pawn with `Is Locally Controlled` before calling `Create Widget`; widgets should never be created for network pawns.
...
#### Split Screen Cursors

- To support split screen, instead create a custom cursor as above and control that instead of using the actual mouse cursor.
- When needed, line-trace through the custom cursor instead.
